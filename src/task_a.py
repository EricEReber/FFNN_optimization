...
for i in range(eta.shape[0]):
    for j in range(lam.shape[0]):
        ...
        error_over_epochs = neural.fit(
            X_train, z_train, Constant, eta[i], lam=lam[j], epochs=1000
        )
        loss_heatmap[i, j] = np.min(error_over_epochs)

# select optimal eta, lambda
i, j = loss_heatmap.argmin()%loss_heatmap.shape[1], loss_heatmap.argmin()//loss_heatmap.shape[1]
min_eta = eta[i]
min_lam = lam[j]

# plot heatmap
ax = sns.heatmap(loss_heatmap, xticklabels=eta, yticklabels=lam, annot=True)
ax.add_patch(Rectangle((i, j), width=1, height=1, fill=False, edgecolor='crimson', lw=4, clip_on=False))
